...
for (let k = 0; k < circlesToConsider; k++) {
  ...
  y += factor * radius * Math.sin(n * baseAngle + phase);
    ...
    </div>`;B.insertAdjacentHTML("beforeend",e),p.push(t)}window.addMoreInputs=(s,t)=>{for(s.disabled=!0;p.length<t;)v()};y("#inputsGroup button",HTMLButtonElement).addEventListener("click",()=>{v()});v();v();{const s=i("segmentCountInput",HTMLInputElement),t=()=>{b.clear();const a=`"use strict";
`+f.value+`
return { x, y };`;let c;try{c=new Function("t /* A value between 0 and 1, inclusive. */","support",a)}catch(o){if(o instanceof SyntaxError){b.displayError(o);return}else throw o}const g=o=>{const d=c(o,H);if(!(Number.isFinite(d.x)&&Number.isFinite(d.y)))throw new Error(`Invalid result.  Expected {x,y} where x and y are both finite numbers.  Found: ${JSON.stringify(d)} when t=${o}.`);return d};let m;try{m=L.parametric(g,s.valueAsNumber)}catch(o){if(o instanceof Error){b.displayError(o);return}else throw o}h.setPathShape(m),C.innerText=h.getOuterHTML()};let e=!1;const n=()=>{w.disabled=!0,e||(e=!0,requestAnimationFrame(()=>{e=!1,t()}))};M.doItSoon=n,w.addEventListener("click",n);const r=i("segmentCountSpan",HTMLSpanElement),l=()=>{r.innerText=s.value.padStart(3,S.FIGURE_SPACE)};l(),s.addEventListener("change",()=>{l(),n()}),window.copyNewInput=(a,c)=>{p[c]=a.valueAsNumber;const g=S.assertClass(a.parentElement?.lastElementChild,HTMLSpanElement);g.innerText=a.valueAsNumber.toFixed(5),n()};{const a=()=>{const c=x[u.selectedIndex];f.value=c.code,n()};u.addEventListener("change",a),i("nextSample",HTMLButtonElement).addEventListener("click",()=>{u.selectedIndex=(u.selectedIndex+1)%x.length,a()})}n()}{const s=i("hide-text",HTMLInputElement);s.addEventListener("click",()=>{s.checked?document.documentElement.dataset.hide="requested":delete document.documentElement.dataset.hide})}
