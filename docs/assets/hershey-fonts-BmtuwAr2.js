import{m as t}from"./misc-BHUyxQfl.js";import{L as g,P as s,Q as G}from"./path-shape-DRm3ZWo5.js";import{a as o}from"./utility-Dgy6CMn3.js";import{D as A}from"./letters-base-6l4xCC6y.js";const c=`12345  1JZ
...
`).forEach(T=>{/^[0-9 ]{8}/.test(T)?X.push(T):X.push(t.assertNonNullable(X.pop())+T)}),X}const E=J(c),B=J(a);function i(n){if(n.length<10||n.length%2==1)throw new Error("wtf");let Z=1/0,L=-1/0,Y=1/0,S=-1/0,V,Q=[];for(let M=10;M<n.length;M+=2){const W=n.substring(M,M+2);if(W==" R")V=void 0;else{const P=W.charCodeAt(0)-82,F=W.charCodeAt(1)-82;if(V){const N=new g(V.x,V.y,P,F);Q.push(N)}V={x:P,y:F},Z=Math.min(Z,P),L=Math.max(L,P),Y=Math.min(Y,F),S=Math.max(S,F)}}const I=n.charCodeAt(8)-82,R=n.charCodeAt(9)-82;return{pathShape:new s(Q),left:Z,right:L,top:Y,bottom:S,leftSideBearing:I,rightSideBearing:R}}const w=E.map(n=>i(n)),y=B.map(n=>i(n));function p(n,X){const T=t.FULL_CIRCLE/4*1.01;function K(S){const Q=S.map((R,M,W)=>{const P=W[M-1],F=W[M+1];if(!(!P&&!F))if(P)if(F){const N=o(R.incomingAngle,P.outgoingAngle),O=o(R.outgoingAngle,F.incomingAngle),e=G.angles(R.x0,R.y0,N,R.x,R.y,O);return e.creationInfo.success?e:void 0}else{if(R.length>=5)return;{const N=o(P.outgoingAngle,R.incomingAngle),O=N-R.incomingAngle,e=R.incomingAngle-O;return G.angles(R.x0,R.y0,N,R.x,R.y,e)}}else{if(R.length>=5)return;{const N=o(R.outgoingAngle,F.incomingAngle),O=N-R.outgoingAngle,e=R.incomingAngle-O;return G.angles(R.x0,R.y0,e,R.x,R.y,N)}}});return Q.forEach((R,M)=>{if(!R)return;const W=Q[M-1],P=S[M-1],F=P&&!W,N=Q[M+1],O=S[M+1],e=O&&!N;if(!(F||e))return;const u=F?P.outgoingAngle:R.incomingAngle,b=e?O.incomingAngle:R.outgoingAngle,H=G.angles(R.x0,R.y0,u,R.x,R.y,b);H.creationInfo.success&&(Q[M]=H)}),Q.map((R,M)=>R||S[M])}const U=[];let Z=[];n.commands.forEach(S=>{if(!(S instanceof g))throw new Error("wtf");const V=Z.at(-1);if(V&&(V.x!=S.x0||V.y!=S.y0||Math.abs(t.angleBetween(V.outgoingAngle,S.incomingAngle))>T)&&(U.push(Z),Z=[]),X==="?")debugger;X==="?"&&Z.length==12&&(U.push(Z),Z=[]),Z.push(S)}),Z.length>0&&U.push(Z);const L=U.map(S=>K(S));return new s(L.flat())}const r=[" ","!",'"',"#","$","%","&","’","(",")","*","+",",","-",".","/",...t.initializedArray(10,n=>String.fromCharCode(n+48)),":",";","<","=",">","?","@",...t.initializedArray(26,n=>String.fromCharCode(n+65)),"[","\\","]","^","_","‘",...t.initializedArray(26,n=>String.fromCharCode(n+97)),"{","|","}","~","▮"],f=r.findIndex(n=>n=="?");if(r[f]!=="?")throw new Error("wtf");console.log("indexOfQuestionMark:",f);function l(n,X){const T=J(n).map(L=>i(L));if(T.length!=r.length)throw new Error("wtf");const K=T[0],U={bottom:X.bottom-X.baseline,capitalTop:0-X.mHeight,defaultKerning:0,mHeight:X.mHeight,top:X.top,strokeWidth:1,spaceWidth:K.rightSideBearing-K.leftSideBearing},Z=new Map;for(const[L,Y]of t.zip(r,T)){const S=new DOMMatrix;S.translateSelf(-Y.leftSideBearing,X.baseline);const V=Y.pathShape.transform(S),Q=p(V,L=="?"?"?":void 0),I=Y.rightSideBearing-Y.leftSideBearing,R=new A(Q,I,U);Z.set(L,R)}return Z}const x=l(c,{baseline:9,bottom:22,top:-17,mHeight:21});l(a,{baseline:9,bottom:17,top:-17,mHeight:21});export{w as c,r as d,y as f,f as i,p as m,x as r};
