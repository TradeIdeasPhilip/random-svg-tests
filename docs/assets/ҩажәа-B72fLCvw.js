...
const y = ySum;
return {x, y};}`}];function lt(a,s){const o=a.getBBox(),t=s.viewBox.baseVal;t.x=o.x,t.y=o.y,t.width=o.width,t.height=o.height;const e=o.width/o.height,n=300,r=n*e;s.style.height=n+"px",s.style.width=r+"px";const i=Math.max(t.width,t.height)/100;return s.style.setProperty("--recommended-stroke-width",i.toString()),{recommendedStrokeWidth:i}}class It{static#e=new this;#n=b.getById("distanceVsT",SVGSVGElement);#t=F("circle[data-distance]",SVGCircleElement,this.#n);#r=F("path",SVGPathElement,this.#n);#s=F("circle[data-t]",SVGCircleElement);constructor(){new b.AnimationLoop(s=>{if(this.#t.style.display="none",this.#s.style.display="none",this.f){s%=5e3;const t=s/5e3,e=t*100;this.#t.style.offsetDistance=e+"%",this.#t.style.display="";const{x:n,y:r}=this.f(t);this.#s.cx.baseVal.value=n,this.#s.cy.baseVal.value=r,this.#s.style.display=""}})}f;update(s,o){this.#r.setAttribute("d",o.rawPath),lt(this.#r,this.#n),this.f=s,this.#t.style.offsetPath=o.cssPath}static update(s,o){this.#e.update(s,o)}}class At{static#e=new this;#n=b.getById("requestedVsReconstructed",SVGSVGElement);#t;#r;#s=F("[data-using] [data-circles]",HTMLTableCellElement);#a=F("[data-using] [data-amplitude]",HTMLTableCellElement);#l=F("[data-adding] [data-circles]",HTMLTableCellElement);#o=F("[data-adding] [data-amplitude]",HTMLTableCellElement);#u=F("[data-available] [data-circles]",HTMLTableCellElement);#i=F("[data-available] [data-amplitude]",HTMLTableCellElement);constructor(){[this.#t,this.#r]=ht("path",SVGPathElement,2,2,this.#n)}#c;update(s,o){this.#c?.();const t=[];this.#c=()=>{t.forEach(d=>d.cancel())},this.#t.setAttribute("d",o.rawPath),lt(this.#t,this.#n);const e=Ft(s),n=bt(e);window.nonZeroTerms=n,window.originalTerms=e;let r=0;n.forEach(d=>r+=d.amplitude);const i=10;let u=0;const l=750,m=500,g=n.slice(0,i-1).flatMap((d,h)=>{const M=d.amplitude/r*100,x={offset:NaN,startTime:NaN,endTime:NaN,usingCircles:h,usingAmplitude:u,addingCircles:1,addingAmplitude:M};u+=M;const E={offset:NaN,startTime:NaN,endTime:NaN,usingCircles:h+1,usingAmplitude:u,addingCircles:0,addingAmplitude:0};return h==0?[E]:[x,E]});{const d=g.at(-1),h=n.length-d.usingCircles;h>0&&g.push({offset:NaN,startTime:NaN,endTime:NaN,usingCircles:d.usingCircles,usingAmplitude:d.usingAmplitude,addingCircles:h,addingAmplitude:100-d.usingAmplitude},{offset:NaN,startTime:NaN,endTime:NaN,usingCircles:n.length,usingAmplitude:100,addingCircles:0,addingAmplitude:0})}let w=NaN;{let d=0;g.forEach(x=>{const E=x.addingCircles?m:l,A=d+E;x.startTime=d,x.endTime=A,d=A}),g.forEach(x=>{x.offset=x.startTime/d}),w=50/d;const M={...g.at(-1),startTime:d,offset:1};g.push(M)}const I={duration:g.at(-1).endTime*3,iterations:1/0};{let d=-1/0,h="";const M=g.map(({offset:x,usingCircles:E})=>{if(E!=d){let A=function(c){if(c.commands.length<2)throw new Error("wtf");const[p,...T]=c.commands,y=T.pop();if(!(p instanceof H&&y instanceof H))throw new Error("wtf");if(p.x0==y.x&&p.y0==y.y)return c;const P=(p.x0+y.x)/2,L=(p.y0+y.y)/2,B=H.controlPoints(P,L,p.x1,p.y1,p.x,p.y),dt=H.controlPoints(y.x0,y.y0,y.x1,y.y1,P,L);return new J([B,...T,dt])};const C=Tt(n,E),R=J.parametric(C,ct);h=A(R).cssPath,d=E}return{offset:x,d:h,easing:"ease-in-out"}});t.push(this.#r.animate(M,I))}{const d=(h,M,x)=>{const E=c=>`'${c.toString().padStart(4,S.FIGURE_SPACE)}'`;let A;const C=new Array;x.forEach(({offset:c,circles:p})=>{A!==void 0&&C.push({offset:c,content:A});const T=A=E(p);C.push({offset:c,content:T})}),t.push(h.animate(C,{pseudoElement:"::after",...I}));const R=x.flatMap(({offset:c,circles:p},T,y)=>{function P(B=p){return B==0?.25:1}if(c==0||c==1)return[{offset:c,opacity:P()}];const L=y[T-1].circles;return L==p?[]:[{offset:c-w,opacity:P(L)},{offset:c,opacity:0},{offset:c+w,opacity:P()}]});[h,M].forEach(c=>t.push(c.animate(R,I)))};d(this.#s,this.#a,g.map(({offset:h,usingCircles:M})=>({offset:h,circles:M}))),d(this.#l,this.#o,g.map(({offset:h,addingCircles:M})=>({offset:h,circles:M}))),d(this.#u,this.#i,g.map(({offset:h,usingCircles:M,addingCircles:x})=>({offset:h,circles:n.length-M-x})))}{let d=function(c){let p;const T=new Array;return g.forEach(y=>{const{offset:P}=y,L=c(y);p!==void 0&&T.push({offset:P,content:p});const B=p=M(L);T.push({offset:P,content:B})}),T};const h=new Intl.NumberFormat("en-US",{minimumSignificantDigits:5,maximumSignificantDigits:5,useGrouping:!1}).format,M=c=>(c<0&&(c=0),h(c)),x=d(c=>c.usingAmplitude),E=d(c=>c.addingAmplitude),A=d(c=>100-c.usingAmplitude-c.addingAmplitude),C=[...x,...E,...A];let R=0;C.forEach(c=>{const[,p,T]=/^([0-9]+)\.([0-9]+)$/.exec(c.content);switch(p.length){case 3:break;case 2:{c.content=S.FIGURE_SPACE+c.content;break}case 1:{c.content=S.FIGURE_SPACE+S.FIGURE_SPACE+c.content;break}default:throw console.warn({beforeDecimalPoint:p,afterDecimalPoint:T,keyframe:c}),new Error("wtf")}R=Math.max(R,c.content.length)}),C.forEach(c=>{c.content=`'${(c.content+"%").padEnd(R+1,S.FIGURE_SPACE)}'`}),t.push(this.#a.animate(x,{pseudoElement:"::after",...I})),t.push(this.#o.animate(E,{pseudoElement:"::after",...I})),t.push(this.#i.animate(A,{pseudoElement:"::after",...I}))}}static update(s,o){this.#e.update(s,o)}}v.addEventListener("input",()=>{_.disabled=!1,O[0].code=v.value,q.selectedIndex=0});q.innerText="";O.forEach((a,s)=>{const o=document.createElement("option");o.innerText=a.name,q.appendChild(o),a.default&&(q.selectedIndex=s,v.value=a.code)});class k{static#e=b.getById("error",HTMLDivElement);static display(s){this.#e.innerText=s}static displayError(s){s instanceof ut?this.#e.innerHTML=`Unable to access <code>support.input(${s.requestedIndex})</code>.  Only ${N.length} input sliders currently exist.  <button onclick="addMoreInputs(this,${s.requestedIndex+1})">Add More</button>`:this.display(s.message)}static clear(){this.display("")}}function Pt(a){try{return J.parametric(a,ct)}catch(s){if(s instanceof Error){k.displayError(s);return}else throw s}}const N=[];class ut extends Error{constructor(s){super(`Unable to access support.input(${s}).  Only ${N.length} input sliders currently exist.`),this.requestedIndex=s}}const ot={input(a){if(!Number.isSafeInteger(a)||a<0)throw new RangeError(`invalid ${a}`);if(a>=N.length)throw new ut(a);return N[a]},ease(a){return(1-Math.cos(Math.PI*a))/2},makeTSplitter:pt,makeTSplitterA:ft,lerpPoints:gt,lerp:S.lerp,makeLinear:S.makeLinear,random:a=>{if(typeof a!="string")throw new RangeError("Invalid seed.  Expecting a string.");return mt.fromString(a)}},Ct=b.getById("inputs",HTMLDivElement);function V(){_.disabled=!1;const a=N.length,s=.5,o=`<div class="has-slider">
    ...
    </div>`;Ct.insertAdjacentHTML("beforeend",o),N.push(s)}window.addMoreInputs=(a,s)=>{for(a.disabled=!0;N.length<s;)V()};F("#inputsGroup button",HTMLButtonElement).addEventListener("click",()=>{V()});V();V();{const a=()=>{k.clear();const t=`"use strict";
`+v.value+`
return f;`;let e;try{e=new Function("support",t)}catch(u){if(u instanceof SyntaxError){k.displayError(u);return}else throw u}let n;try{n=e(ot)}catch(u){if(u instanceof Error){k.displayError(u);return}else throw u}const r=u=>{const l=n(u,ot);if(!(Number.isFinite(l.x)&&Number.isFinite(l.y)))throw new Error(`Invalid result.  Expected {x,y} where x and y are both finite numbers.  Found: ${JSON.stringify(l)} when t=${u}.`);return l},i=Pt(r);i&&(It.update(r,i),At.update(r,i))};let s=!1;const o=()=>{_.disabled=!0,s||(s=!0,requestAnimationFrame(()=>{s=!1,a()}))};_.addEventListener("click",o),window.copyNewInput=(t,e)=>{N[e]=t.valueAsNumber;const n=S.assertClass(t.parentElement?.lastElementChild,HTMLSpanElement);n.innerText=t.valueAsNumber.toFixed(5),o()};{const t=()=>{const e=O[q.selectedIndex];v.value=e.code,o()};q.addEventListener("change",t),b.getById("nextSample",HTMLButtonElement).addEventListener("click",()=>{q.selectedIndex=(q.selectedIndex+1)%O.length,t()})}{const t=b.getById("codeSamplesHolder",HTMLDivElement),e=b.getById("inputsGroup",HTMLDivElement),n=`<div>
            <div data-description>
              <button class="show-this">Show This</button><span></span>
            </div>
            <pre data-code-snippet></pre></div>`;O.forEach((r,i)=>{if(i>0){t.insertAdjacentHTML("beforeend",n);const u=t.lastElementChild,l=F("span",HTMLSpanElement,u);l.innerText=r.name;const m=F("pre",HTMLPreElement,u);m.innerText=r.code,F("button",HTMLButtonElement,u).addEventListener("click",()=>{v.value=r.code,o(),e.scrollIntoView({behavior:"smooth"})})}})}o()}{const a=b.getById("hide-text",HTMLInputElement);a.addEventListener("click",()=>{a.checked?document.documentElement.dataset.hide="requested":delete document.documentElement.dataset.hide})}
