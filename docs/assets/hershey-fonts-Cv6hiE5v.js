import{m as K}from"./misc-BHUyxQfl.js";import{L as H,P as c,Q as I}from"./path-shape-CZZKMsju.js";import{a as G}from"./utility-Dgy6CMn3.js";import{D as u}from"./letters-base-CTGJGQGt.js";const s=`12345  1JZ
...
`).forEach(T=>{/^[0-9 ]{8}/.test(T)?Z.push(T):Z.push(K.assertNonNullable(Z.pop())+T)}),Z}const b=o(s),A=o(g);function r(n){if(n.length<10||n.length%2==1)throw new Error("wtf");let O=1/0,U=-1/0,S=1/0,P=-1/0,V,t=[];for(let Y=10;Y<n.length;Y+=2){const N=n.substring(Y,Y+2);if(N==" R")V=void 0;else{const F=N.charCodeAt(0)-82,M=N.charCodeAt(1)-82;if(V){const L=new H(V.x,V.y,F,M);t.push(L)}V={x:F,y:M},O=Math.min(O,F),U=Math.max(U,F),S=Math.min(S,M),P=Math.max(P,M)}}const R=n.charCodeAt(8)-82,X=n.charCodeAt(9)-82;return{pathShape:new c(t),left:O,right:U,top:S,bottom:P,leftSideBearing:R,rightSideBearing:X}}const d=b.map(n=>r(n)),D=A.map(n=>r(n));function B(n,Z=K.FULL_CIRCLE/4*1.01){function T(S){const V=S.map((R,X,Y)=>{const N=Y[X-1],F=Y[X+1];if(!(!N&&!F))if(N)if(F){const M=G(R.incomingAngle,N.outgoingAngle),L=G(R.outgoingAngle,F.incomingAngle),W=I.angles(R.x0,R.y0,M,R.x,R.y,L);return W.creationInfo.success?W:void 0}else{if(R.length>=5)return;{const M=G(N.outgoingAngle,R.incomingAngle),L=M-R.incomingAngle,W=R.incomingAngle-L;return I.angles(R.x0,R.y0,M,R.x,R.y,W)}}else{if(R.length>=5)return;{const M=G(R.outgoingAngle,F.incomingAngle),L=M-R.outgoingAngle,W=R.incomingAngle-L;return I.angles(R.x0,R.y0,W,R.x,R.y,M)}}});return V.forEach((R,X)=>{if(!R)return;const Y=V[X-1],N=S[X-1],F=N&&!Y,M=V[X+1],L=S[X+1],W=L&&!M;if(!(F||W))return;const f=F?N.outgoingAngle:R.incomingAngle,l=W?L.incomingAngle:R.outgoingAngle,J=I.angles(R.x0,R.y0,f,R.x,R.y,l);J.creationInfo.success&&(V[X]=J)}),V.map((R,X)=>R||S[X])}const e=[];let Q=[];n.commands.forEach(S=>{if(!(S instanceof H))throw new Error("wtf");const P=Q.at(-1);P&&(P.x!=S.x0||P.y!=S.y0||Math.abs(K.angleBetween(P.outgoingAngle,S.incomingAngle))>Z)&&(e.push(Q),Q=[]),Q.push(S)}),Q.length>0&&e.push(Q);const O=e.map(S=>T(S));return new c(O.flat())}const i=[" ","!",'"',"#","$","%","&","’","(",")","*","+",",","-",".","/",...K.initializedArray(10,n=>String.fromCharCode(n+48)),":",";","<","=",">","?","@",...K.initializedArray(26,n=>String.fromCharCode(n+65)),"[","\\","]","^","_","‘",...K.initializedArray(26,n=>String.fromCharCode(n+97)),"{","|","}","~","▮"];function a(n,Z){const T=o(n).map(U=>r(U));if(T.length!=i.length)throw new Error("wtf");const e=T[0],Q={bottom:Z.bottom-Z.baseline,capitalTop:0-Z.mHeight,defaultKerning:0,mHeight:Z.mHeight,top:Z.top,strokeWidth:1,spaceWidth:e.rightSideBearing-e.leftSideBearing},O=new Map;for(const[U,S]of K.zip(i,T)){const P=new DOMMatrix;P.translateSelf(-S.leftSideBearing,Z.baseline);const V=S.pathShape.transform(P),t=B(V),R=S.rightSideBearing-S.leftSideBearing,X=new u(t,R,Q);O.set(U,X)}return O}const w=a(s,{baseline:9,bottom:22,top:-17,mHeight:21}),y=a(g,{baseline:9,bottom:17,top:-17,mHeight:21});export{y as a,d as c,i as d,D as f,B as m,w as r};
